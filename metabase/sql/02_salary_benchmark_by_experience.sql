...
clean_data AS (
    ...
    LEFT JOIN raw_mart.dim_job j ON f.job_sk = j.job_sk
...
SELECT 
    experience AS "Cấp bậc",
    COUNT(*) AS "Số lượng tin",
    ROUND(AVG((salary_min + salary_max) / 2.0) / 1000000, 2) AS "Mức lương trung bình (triệu VNĐ)",
    ROUND(AVG(salary_min) / 1000000, 2) AS "Mức lương khởi điểm trung bình (triệu VNĐ)",
    ROUND(AVG(salary_max) / 1000000, 2) AS "Mức lương trần trung bình (triệu VNĐ)"
FROM clean_data
GROUP BY experience
ORDER BY
    -- sort_index không SELECT ra ngoài nữa nên không còn bị vẽ thành series
    CASE 
        ...
        WHEN lower(experience) LIKE '%trên 5 năm%' THEN 7
